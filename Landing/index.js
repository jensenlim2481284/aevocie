...
$(document).on('mouseover','.nav-btn',function(){
  ...
  class AnimatedText extends Object3D {
    constructor(text, font, size =0.3, letterSpacing = 0.03, color = '#ffffff') {
      super();
  ...
  setTimeout(() => {
    const img = new AnimatedImage('assets/img/logo.png');
  ...
  setTimeout(() => {
    const text = new AnimatedText('Æ Voice', fontAsset);
  ...
  setTimeout(() => {
    const text = new AnimatedText('Reinvent the way of advertising.', fontAsset, 0.125);
    text.position.x -= text.basePosition * 0.5;
    text.position.y -= 0.8;
    webgl.add(text);
    text.show();
